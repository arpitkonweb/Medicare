...
router.post('/',upload.single('image'), (request, response) => {
    ...
    const connection = db.connect1()
    const statement = `
    INSERT INTO products (name, price, discount, priceWithDiscount, doseInMG, mgfdate, expiredate, description,image) values('${name}', ${price}, ${discount}, ${priceWithDiscount}, ${doseInMG}, '${mgfdate}', '${expiredate}', '${description}','${file}')`
    connection.query(statement, (error, data) => {
        connection.end()
        response.send(utils.createResult(error, data))
    })
